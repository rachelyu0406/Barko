...
import { Newspaper, ExternalLink, Calendar, TrendingUp, Loader, AlertCircle } from 'lucide-react';
...
export function News({ country = 'US', userLocation = 'United States' }: NewsProps) {
  const [articles, setArticles] = useState<NewsArticle[]>([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);
  const [category, setCategory] = useState<'general' | 'business' | 'markets'>('business');
    ...
    return [
      {
        ...
        description: 'The Federal Reserve announced it will maintain current interest rates as inflation shows signs of cooling while employment remains strong.',
      ...
      },
      {
        ...
        description: 'A recent survey reveals that 60% of Americans would struggle to cover a $1,000 emergency expense, highlighting the importance of financial literacy.',
      ...
      },
      {
        ...
        description: 'Major indices climbed to record levels as technology stocks led the rally, with investors showing renewed confidence in growth sectors.',
      ...
      },
      {
        ...
        description: 'Home sales declined for the third consecutive month as higher mortgage rates continue to impact affordability for first-time buyers.',
      ...
      {
        title: 'Cryptocurrency Market Volatility Continues Amid Regulatory Concerns',
        description: 'Bitcoin and major cryptocurrencies experienced significant price swings as investors await clarity on new regulatory frameworks.',
      ...
      },
      {
        ...
        description: 'New research indicates millennials and Gen Z workers are falling behind on retirement savings compared to previous generations at the same age.',
  ...
  const fetchNews = async () => {
    ...
    try {
      ...
      const url = `https://newsapi.org/v2/top-headlines?country=${countryCode}&category=business&q=${encodeURIComponent(query)}&pageSize=12&apiKey=${apiKey}`;
  ...
  console.log('🎨 Rendering News component - articles:', articles.length, 'loading:', loading);
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-blue-300 text-sm">Stay informed with the latest financial updates</p>
        </div>
      </div>
